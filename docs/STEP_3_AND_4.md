...
- **Site URL:** `https://winstonac.github.io/Winston Fit/` (adjust if your URL differs; match how you open the site).
...
3. In the Winston Fit repo, edit **`winston-fit-config.js`** at the project root to look exactly like this (replace the two quoted values):
...
1. Open your **live** Winston Fit URL in a browser (or incognito).
2. **Overview** → **Sign in** (or scroll to **Settings → Sign in**).
3. Enter an email → **Send link** → use the email → **Enter Winston Fit** → you should be signed in.

**Multiple users:** each person uses their own email; each gets their own auth user and (via RLS) their own `winstonfit_profiles` row. No extra code — same deployed app for everyone.
