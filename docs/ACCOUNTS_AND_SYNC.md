# Winston Fit: accounts, sync, and roadmap
...
| **B — Optional Supabase** | Same person on multiple devices, or you want a **backup** and **per-user rows** to inspect later | Email magic link | Browser + row in `winstonfit_profiles` |
...
**You do not need Vercel** to use Supabase with Winston Fit on **GitHub Pages**.
...
- **Site URL:** your live app root, e.g. `https://winstonac.github.io/Winston Fit/`
...
3. Deploy `winston-fit-config.js` with the site.
...
- On **save** (completions, profile, etc.), the app still updates **`localStorage`** and **debounces** an **upsert** to `winstonfit_profiles` when signed in.
...
- **Later:** optional **anonymous** event table (e.g. `winstonfit_events` with `user_id`, `event`, `payload`, `created_at`) and RLS so users only insert their own rows—or use **Supabase Logs / Edge Functions** if you outgrow SQL queries.
